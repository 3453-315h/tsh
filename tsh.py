    ...
    try:
        ...
        if not db_table_exists(db_connection, 'MessageTarget'):
            ...
            c.execute('''INSERT INTO MessageTarget VALUES (?, ?)''',
                      ('msg.fifo', json.dumps(config.senders)))
            c.execute('''INSERT INTO MessageTarget VALUES (?, ?)''',
...
def db_target_get(source):
    ...
    c = db_connection.cursor()
    query = \
        c.execute('''SELECT destination FROM MessageTarget WHERE source=?'''
                  , (source, ))
    result = query.fetchone()
...
def db_target_get_all():
    ...
    c = db_connection.cursor()
    query = \
        c.execute('''SELECT source, destination FROM MessageTarget''')
    sources = {}
    for source in query:
...
def db_target_redirect(source, target):
    db_connection = sqlite3.connect('config.db')
    c = db_connection.cursor()
    c.execute('''INSERT OR REPLACE INTO MessageTarget (source, destination) VALUES (?, ?)'''
              , (source, json.dumps(target)))
    db_connection.commit()
    db_connection.close()
...
def db_source_add(source):
    db_connection = sqlite3.connect('config.db')
    c = db_connection.cursor()
    c.execute('''INSERT INTO MessageTarget VALUES (?, ?)''', (source
              + '.fifo', json.dumps(config.senders)))
    c.execute('''INSERT INTO MessageTarget VALUES (?, ?)''', (source
              + '.socket', json.dumps(config.senders)))
    db_connection.commit()
    db_connection.close()
...
def db_source_del(source):
    db_connection = sqlite3.connect('config.db')
    c = db_connection.cursor()
    c.execute('''DELETE FROM MessageTarget WHERE source=?''', (source
              + '.fifo', ))
    c.execute('''DELETE FROM MessageTarget WHERE source=?''', (source
              + '.socket', ))
    db_connection.commit()
...
def db_source_get(type):
    ...
    c = db_connection.cursor()
    query = \
        c.execute('''SELECT source, destination FROM MessageTarget WHERE source LIKE ?'''
                  , ('%.{}'.format(type), ))
    for source in query:
...
def db_chat_add(id, type, name):
    ...
    db_connection = sqlite3.connect('config.db')
    c = db_connection.cursor()
    c.execute('''INSERT OR REPLACE INTO KnownChats (id, type, name) VALUES (?, ?, ?)'''
              , (id, type, name))
    db_connection.commit()
    db_connection.close()
    ...
    while True:
        ...
        for fifo_file in fifos:
            if not os.path.isfile(fifo_file) \
                or not stat.S_ISFIFO(os.stat(fifo_file).st_mode):
                try:
                    os.unlink(fifo_file)
                except OSError, e:
                    if e.errno == os.errno.ENOENT:
...
def proc_run(command):
    ...
    watcher = Thread(target=proc_kill_on_timeout, args=(done, timeout,
                     proc))
    watcher.daemon = True
    watcher.start()
...
def handle(msg):
    ...
    db_chat_add(chat_id, msg['chat']['type'], (msg['chat']['title'
                ] if 'title' in msg['chat'] else username))
